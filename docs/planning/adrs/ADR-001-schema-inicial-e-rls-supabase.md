...
- **Cadastro pré-login** (F-03/F-32/F-33): solicitações de cadastro existem *antes* de haver `auth.users`, logo não podem depender de `auth.uid()`.
...
- Sobreposição de horário: o CHECK garante `start_time < end_time`, mas a *ausência de conflito* entre reservas aprovadas não é imposta por constraint neste ADR (exigiria `EXCLUDE USING gist` com `btree_gist`). Decisão: validar na aplicação por ora; constraint de exclusão fica como evolução proposta.
