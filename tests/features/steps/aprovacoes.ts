...
Given(
  "que o administrador acessa o painel de indicadores",
  function (this: SiraWorld) {
    this.currentUser = ensureUser(this, "Admin", { role: "admin" });
  },
);
...
Given(
  "que o administrador está com o painel aberto",
  function (this: SiraWorld) {
    this.currentUser = ensureUser(this, "Admin", { role: "admin" });
    this.focusReservation = addReservation(this, {
      owner: "Ana",
...
When(
  "não há nenhuma reserva no período selecionado",
  function (this: SiraWorld) {
    const counts: StatusCounts = {
      pending: 0,
      approved: 0,
...
When(
  "uma reserva da professora Ana para o Lab 1 é aprovada",
  function (this: SiraWorld) {
    const r = this.focusReservation!;
    r.status = "approved";
    this.results = {
      approved: this.reservations.filter((x) => x.status === "approved").length,
    };
  },
);

Then(
  "o painel mostra o total de reservas, a taxa de aprovação, as salas mais ocupadas e os professores ativos",
  function (this: SiraWorld) {
    const kpi = this.results as {
      total: number;
      rate: number | null;
      rooms: unknown[];
...
Then(
  "exibe um aviso de ausência de dados no período",
  function (this: SiraWorld) {
...
Then(
  "os indicadores do painel passam a refletir a mudança sem recarregar a tela",
  function (this: SiraWorld) {
...
When(
  ...
  function (this: SiraWorld, name: string, capacity: string) {
    ...
    if (result.ok && result.value) {
      ensureRoom(this, result.value.name, {
...
Given(
  ...
  function (this: SiraWorld, active: string, inactive: string) {
    this.currentUser = ensureUser(this, "Admin", { role: "admin" });
...
When(
  ...
  function (this: SiraWorld, resource: string) {
    ...
    this.results = active.filter((r: WorldRoom) =>
...
Then(
  ...
  function (this: SiraWorld) {
    ...
    assert.ok(
      rooms.every(
        (r) => typeof r.capacity === "number" && Array.isArray(r.resources),
      ),
    );
  },
);
...
Then(
  ...
  function (this: SiraWorld) {
    ...
    const counts = rooms.map(
      (r) => this.reservations.filter((x) => x.roomName === r.name).length,
    );
...
When(
  ...
  function (this: SiraWorld, capacity: string) {
    ...
    const result = validateRoomInput({
      ...
      capacity: Number(capacity),
      block: "B",
      resources: ["datashow", "ar-condicionado"],
      status: "active",
    });
    if (result.ok && result.value) {
  ...
  function (this: SiraWorld) {
    assert.ok(
      this.reservations.some(
        (r) => r.owner === "Ana" && r.status === "approved",
      ),
    );
  },
);
...
When(
  ...
  function (this: SiraWorld) {
    const room = this.focusRoom!;
    const hasFuture = this.reservations.some(
      (r) =>
...
When(
  ...
  function (this: SiraWorld, name: string) {
    ...
    if (hasFuture) {
      this.actionOk = false;
      this.message =
        "É preciso migrar ou cancelar as reservas antes de excluir.";
    } else {
      this.rooms = this.rooms.filter((r) => r.id !== room.id);
      this.actionOk = true;
    }
