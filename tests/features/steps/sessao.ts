...
Given("que Ana tem uma conta ativa com o e-mail {string}", function (this: SiraWorld, email: string) {
  this.currentUser = ensureUser(this, "Ana", { email, status: "active" });
});

Given("que Ana tem uma conta inativa com o e-mail {string}", function (this: SiraWorld, email: string) {
...
When("ela informa o e-mail {string} e a senha correta e confirma o acesso", function (this: SiraWorld, email: string) {
...
When("ele informa o e-mail {string} e uma senha e confirma o acesso", function (this: SiraWorld, email: string) {
  if (!isInstitutionalEmail(email)) {
    this.actionOk = false;
    this.message = "Somente e-mails institucionais são aceitos.";
  } else {
...
Then("o sistema a leva para a tela inicial do perfil dela", function (this: SiraWorld) {
...
Then("exibe um aviso de que somente e-mails institucionais são aceitos", function (this: SiraWorld) {
...
Given("que Ana está na tela de acesso e abre o formulário de solicitação de cadastro", function (this: SiraWorld) {
  this.sessionActive = false;
});

Given("que Bruno abriu o formulário de solicitação de cadastro", function (this: SiraWorld) {
  this.sessionActive = false;
});

Given("que a solicitação de Ana com o e-mail {string} está pendente de aprovação", function (this: SiraWorld, email: string) {
  ensureUser(this, "Ana", { email, pendingSignup: true, status: "inactive" });
});

When("ela informa nome, o e-mail {string}, o departamento e uma senha e envia", function (this: SiraWorld, email: string) {
  if (!isInstitutionalEmail(email)) {
    this.actionOk = false;
    this.message = "Somente e-mails institucionais são aceitos.";
  } else {
...
When("ele informa o e-mail {string} e envia", function (this: SiraWorld, email: string) {
  if (!isInstitutionalEmail(email)) {
    this.actionOk = false;
    this.message = "Somente e-mails institucionais são aceitos.";
  } else {
...
When("ela tenta entrar no sistema com esse e-mail e a senha informada", function (this: SiraWorld) {
...
Then("o sistema registra a solicitação como pendente de aprovação", function (this: SiraWorld) {
  assert.equal(this.actionOk, true);
  assert.ok(this.users.some((u) => u.pendingSignup));
});

Then("avisa Ana de que o pedido aguarda análise do administrador", function (this: SiraWorld) {
...
Then("informa que o cadastro ainda aguarda aprovação", function (this: SiraWorld) {
  assert.match(this.message ?? "", /aguarda aprovação/i);
});

// ─────────────────────────── US02 — Continuidade de sessão ──────────────────
Given("que Ana entrou no sistema e está na tela inicial", function (this: SiraWorld) {
...
When("ele fecha e reabre o navegador enquanto a sessão ainda é válida", function (this: SiraWorld) {
  // Sessão válida sobrevive ao fechamento do navegador (F-02 CA03).
  this.sessionActive = this.currentUser !== null;
});

Then("ela continua conectada sem precisar informar e-mail e senha novamente", function (this: SiraWorld) {
  assert.equal(this.sessionActive, true);
});
...
Then("mantém o perfil de professor e as mesmas permissões de antes", function (this: SiraWorld) {
...
Given("que Ana e Bruno já usaram o sistema no mesmo computador", function (this: SiraWorld) {
  ensureUser(this, "Ana");
  ensureUser(this, "Bruno");
  addReservation(this, { owner: "Bruno", roomName: "Lab 1", status: "approved" });
...
Then("os dados pessoais da sessão de Ana deixam de ficar visíveis na tela", function (this: SiraWorld) {
  assert.equal(this.results, null);
});

Then("os dados pessoais de Bruno permanecem preservados", function (this: SiraWorld) {
  assert.ok(this.users.some((u) => u.name === "Bruno"));
  assert.ok(this.reservations.some((r) => r.owner === "Bruno"));
});

Then("ficam disponíveis quando ele entrar novamente", function (this: SiraWorld) {
...
Given("que Bruno entrou no sistema como administrador", function (this: SiraWorld) {
...
When("ela observa o item Notificações no menu lateral", function (this: SiraWorld) {
  this.results = this.notifications.filter((n) => !n.is_read).length;
});

Then("ela vê Minhas Reservas, Nova Reserva, Calendário e Notificações", function (this: SiraWorld) {
  const menu = this.results as string[];
  for (const item of PROFESSOR_MENU) assert.ok(menu.includes(item), `falta ${item}`);
});

Then("não vê as seções de gestão do administrador", function (this: SiraWorld) {
  const menu = this.results as string[];
  for (const item of ADMIN_ONLY_MENU) assert.ok(!menu.includes(item), `não devia ver ${item}`);
});

Then("ele vê as seções Salas, Usuários, Aprovações e Painel", function (this: SiraWorld) {
  const menu = this.results as string[];
  for (const item of ADMIN_ONLY_MENU) assert.ok(menu.includes(item), `falta ${item}`);
});
...
Then("reflete a quantidade real de pendências", function (this: SiraWorld) {
  assert.equal(this.results, this.notifications.filter((n) => !n.is_read).length);
});

// ─────────────────────────── US06 — Endereços das telas ─────────────────────
Given("que Ana visitou a tela de Calendário e depois a tela de Nova Reserva", function (this: SiraWorld) {
...
When("ele tenta abrir um endereço que não corresponde a nenhuma tela", function (this: SiraWorld) {
  this.results = "404";
});

Then("o sistema a leva de volta à tela de Calendário", function (this: SiraWorld) {
  assert.equal(this.navHistory[this.navIndex], "/calendario");
});

Then("o comando de avançar a traz novamente à tela de Nova Reserva", function (this: SiraWorld) {
  if (this.navIndex < this.navHistory.length - 1) this.navIndex++;
  assert.equal(this.navHistory[this.navIndex], "/reservas/nova");
});

Then("o sistema apresenta a tela de página não encontrada", function (this: SiraWorld) {
  assert.equal(this.results, "404");
});

Then("o sistema a mantém na tela de Minhas Reservas", function (this: SiraWorld) {
...
Given("que Ana está no celular com o menu recolhido", function (this: SiraWorld) {
  this.viewportWidth = 390;
  this.menuOpen = false;
});

Given("que Ana está no celular com o menu lateral aberto", function (this: SiraWorld) {
  this.viewportWidth = 390;
  this.menuOpen = true;
});
...
Then("o conteúdo principal ocupa o espaço disponível", function (this: SiraWorld) {
...
Given("que Ana acessa Minhas Reservas pelo celular", function (this: SiraWorld) {
  this.viewportWidth = 390;
});

Given("que Bruno consulta a lista de Aprovações pelo celular", function (this: SiraWorld) {
  this.viewportWidth = 390;
});

Given("que Ana acessa a lista de Salas em um computador de tela grande", function (this: SiraWorld) {
...
Then("cada reserva aparece como um cartão empilhado", function (this: SiraWorld) {
  assert.equal(this.results, "cards");
});

Then("cada valor exibe o rótulo do campo correspondente", function (this: SiraWorld) {
  assert.equal(this.results, "cards");
});

Then("não é necessário rolar a tela para o lado para ver as informações", function (this: SiraWorld) {
  assert.equal(this.results, "cards");
});

Then("as informações voltam a aparecer no formato de tabela", function (this: SiraWorld) {
  assert.equal(this.results, "table");
});

// ─────────────────────────── US09 — Tema da interface ───────────────────────
Given("que Ana está usando o sistema no tema claro", function (this: SiraWorld) {
  this.theme = "light";
});

Given("que Bruno deixou o tema escuro definido no acesso anterior", function (this: SiraWorld) {
  this.persistedTheme = "dark";
});

Given("que Ana ativou o tema escuro na tela inicial", function (this: SiraWorld) {
  this.theme = "dark";
  this.persistedTheme = "dark";
});

When("ela aciona o controle de troca de tema para escuro", function (this: SiraWorld) {
  this.theme = "dark";
  this.persistedTheme = "dark";
});
...
When("ela navega para a tela de minhas reservas e para o calendário", function (this: SiraWorld) {
  this.navHistory = ["/reservas", "/calendario"];
});

Then("toda a interface passa a ser exibida no tema escuro", function (this: SiraWorld) {
  assert.equal(this.theme, "dark");
});

Then("ao entrar novamente no sistema o tema escuro continua aplicado", function (this: SiraWorld) {
...
Then("não há nenhum instante em que a cor clara errada é mostrada", function (this: SiraWorld) {
  assert.equal(this.themeAppliedBeforePaint, true);
});

Then("todas essas telas são exibidas no tema escuro de forma consistente", function (this: SiraWorld) {
  assert.equal(this.theme, "dark");
  assert.ok(this.navHistory.length >= 2);
});

// ─────────────────────────── US10 — Privacidade / resiliência ───────────────
Given("que a professora {string} está logada e possui reservas cadastradas", function (this: SiraWorld, name: string) {
...
Given("o professor {string} usa o mesmo navegador", function (this: SiraWorld, name: string) {
  ensureUser(this, name);
});

Given("que {string} possui reservas e está logada", function (this: SiraWorld, name: string) {
  ensureUser(this, name);
  addReservation(this, { owner: name, roomName: "Lab 1", status: "approved" });
  this.currentUser = this.users.find((u) => u.name === name) ?? null;
});

Given("que o armazenamento do navegador está cheio", function (this: SiraWorld) {
  this.results = "storage-full";
});

Given("que o usuário {string} está logado", function (this: SiraWorld, name: string) {
...
When("{string} sai do sistema e entra novamente", function (this: SiraWorld, name: string) {
  this.currentUser = null;
  this.currentUser = this.users.find((u) => u.name === name) ?? null;
  this.results = this.reservations.filter((r) => r.owner === name);
});
...
When("o usuário tenta salvar uma nova reserva", function (this: SiraWorld) {
  ...
  if (this.results === "storage-full") {
    this.message = "Seu armazenamento está cheio. Libere espaço e tente de novo.";
    this.actionOk = false;
  }
});

When("o sistema tenta acessar um dado pertencente a {string}", function (this: SiraWorld, owner: string) {
  // Acesso a dado de outro dono é barrado pelo RLS → nada retorna.
  const visible = this.reservations.filter((r) => r.owner === this.currentUser?.name);
  this.results = visible.filter((r) => r.owner === owner);
});

Then("o sistema exibe apenas as reservas de {string}", function (this: SiraWorld, name: string) {
  const rows = this.results as { owner: string }[];
  assert.ok(rows.every((r) => r.owner === name));
});

Then("nenhuma reserva de {string} aparece", function (this: SiraWorld, name: string) {
  const rows = this.results as { owner: string }[];
  assert.ok(rows.every((r) => r.owner !== name));
});

Then("suas reservas anteriores continuam disponíveis", function (this: SiraWorld) {
  const rows = this.results as unknown[];
  assert.ok(rows.length > 0);
});

Then("o sistema exibe uma mensagem amigável de armazenamento cheio", function (this: SiraWorld) {
