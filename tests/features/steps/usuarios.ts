...
import { ensureUser, addReservation, addNotification } from "../support/world";
...
Given(
  "que já existe um usuário com o e-mail {string}",
...
When(
  "eu informo o nome {string}, o e-mail {string}, o perfil de professor e uma senha inicial",
  function (this: SiraWorld, name: string, email: string) {
...
When(
  "eu tento criar um novo usuário com o e-mail {string}",
  function (this: SiraWorld, email: string) {
...
When(
  "eu informo o nome {string}, o e-mail {string} e a senha inicial, mas não escolho o perfil",
  function (this: SiraWorld, name: string, email: string) {
    const result = validateUserInput(
...
Then(
  "já fica ativo, sem passar pela aprovação de solicitação",
...
Then(
  "sou avisado de que já existe um usuário com esse e-mail",
  function (this: SiraWorld) {
...
Given("que estou na tela de usuários cadastrados", function (this: SiraWorld) {
  this.currentUser = ensureUser(this, "Admin", { role: "admin" });
  ensureUser(this, "Ana", { role: "professor", email: "ana@ifpb.edu.br" });
  ensureUser(this, "Carlos", {
    role: "professor",
    email: "carlos@ifpb.edu.br",
  });
  ensureUser(this, "Diretora", {
    role: "admin",
    email: "diretora@ifpb.edu.br",
  });
});
...
Then(
  "vejo todos os usuários com nome, e-mail e perfil",
  function (this: SiraWorld) {
    const rows = this.results as {
      full_name: string;
      email: string;
      role: string;
    }[];
...
Then(
  "cada usuário oferece as ações de editar e excluir",
  function (this: SiraWorld) {
...
Then(
  "vejo um aviso de que nenhum usuário foi encontrado",
  function (this: SiraWorld) {
    assert.equal((this.results as unknown[]).length, 0);
  },
);
...
Then(
  "vejo apenas os usuários com perfil de professor",
  function (this: SiraWorld) {
...
Then(
  "o usuário {string}, de perfil professor, aparece na lista",
...
Given(
  "que estou editando o usuário {string}",
  function (this: SiraWorld, name: string) {
    this.currentUser = ensureUser(this, "Admin", { role: "admin" });
    ...
    this.focusUser = ensureUser(this, name, {
      role: "professor",
      email: `${name.toLowerCase()}@ifpb.edu.br`,
    });
  },
);

When(
  "troco o nome para {string} e o perfil para administrador",
  function (this: SiraWorld, newName: string) {
...
When("abro o formulário de edição", function (this: SiraWorld) {
  ...
  this.results = {
    editableFields: ["nome", "perfil", "senha"],
    emailLocked: true,
  };
});
...
When("informo uma nova senha e confirmo", function (this: SiraWorld) {
  const result = validateUserInput(
    {
      fullName: this.focusUser!.name,
      role: this.focusUser!.role,
...
Then(
  "os novos dados de {string} ficam salvos",
  function (this: SiraWorld, name: string) {
    assert.equal(this.actionOk, true);
...
Then(
  "o campo de e-mail aparece bloqueado para alteração",
  function (this: SiraWorld) {
...
Then(
  "a nova senha de {string} passa a valer no próximo acesso dele",
  function (this: SiraWorld, _name: string) {
    assert.equal(this.actionOk, true);
  },
);
...
Given(
  "que estou prestes a excluir o usuário {string}",
  function (this: SiraWorld, name: string) {
    this.currentUser = ensureUser(this, "Admin", { role: "admin" });
...
Given(
  "que iniciei a exclusão do usuário {string}",
  function (this: SiraWorld, name: string) {
    this.currentUser = ensureUser(this, "Admin", { role: "admin" });
    this.focusUser = ensureUser(this, name, { role: "professor" });
  },
...
Given(
  "que o usuário {string} possui um histórico extenso de reservas",
  function (this: SiraWorld, name: string) {
    this.currentUser = ensureUser(this, "Admin", { role: "admin" });
    this.focusUser = ensureUser(this, name, { role: "professor" });
...
Then(
  "{string} deixa de constar na lista de usuários",
  function (this: SiraWorld, name: string) {
    assert.ok(!this.users.some((u) => u.name === name));
  },
);
...
Then(
  "{string} não consegue mais acessar o sistema",
  function (this: SiraWorld, name: string) {
    assert.ok(
      !this.users.some((u) => u.name === name && u.status === "active"),
    );
  },
);

Then(
  "as reservas de {string} continuam preservadas e marcadas",
  function (this: SiraWorld, name: string) {
...
Then(
  "{string} continua cadastrada e com acesso normal",
  function (this: SiraWorld, name: string) {
...
Then(
  "o sistema recomenda desativar a conta em vez de excluí-la",
  function (this: SiraWorld) {
...
Given(
  "que existe uma solicitação pendente de {string}, com e-mail {string}",
  function (this: SiraWorld, name: string, email: string) {
    this.currentUser = ensureUser(this, "Admin", { role: "admin" });
    this.focusUser = ensureUser(this, name, {
      email,
...
Given(
  "que não há solicitações de cadastro pendentes",
  function (this: SiraWorld) {
    this.currentUser = ensureUser(this, "Admin", { role: "admin" });
  },
);
...
When("aprovo a solicitação", function (this: SiraWorld) {
  ...
  addNotification(this, {
    ...
    message: "Você já pode entrar.",
  });
  this.actionOk = true;
});

When(
  "tento abrir a fila de solicitações pendentes",
  function (this: SiraWorld) {
...
Then(
  "o usuário {string} é criado e habilitado a acessar",
  function (this: SiraWorld, name: string) {
...
Then(
  "{string} é avisada de que já pode entrar",
  function (this: SiraWorld, name: string) {
    assert.ok(
      this.notifications.some(
        (n) => n.owner === name && /já pode entrar/i.test(n.message),
      ),
    );
  },
);
...
Then("a solicitação sai da fila de pendentes", function (this: SiraWorld) {
  assert.ok(
    !this.users.some((u) => u.pendingSignup && u.id === this.focusUser?.id),
  );
});

Then(
  "a ação de aprovar não fica disponível para mim",
  function (this: SiraWorld) {
...
Then(
  "vejo um aviso de que não há solicitações a aprovar",
  function (this: SiraWorld) {
    assert.equal((this.results as unknown[]).length, 0);
  },
);
...
Given(
  "que existe uma solicitação pendente de {string}",
  function (this: SiraWorld, name: string) {
    this.currentUser = ensureUser(this, "Admin", { role: "admin" });
    this.focusUser = ensureUser(this, name, {
      pendingSignup: true,
...
Given(
  "que estou recusando a solicitação de {string}",
  function (this: SiraWorld, name: string) {
    this.currentUser = ensureUser(this, "Admin", { role: "admin" });
    this.focusUser = ensureUser(this, name, {
      pendingSignup: true,
...
Given(
  "que a solicitação de {string} foi recusada com o motivo {string}",
  function (this: SiraWorld, name: string, reason: string) {
    this.focusUser = ensureUser(this, name, {
      pendingSignup: true,
      status: "inactive",
      signupReason: reason,
...
When(
  "recuso a solicitação informando o motivo {string}",
...
When(
  "confirmo a recusa sem escrever uma justificativa",
  function (this: SiraWorld) {
...
When(
  "{string} verifica o aviso recebido",
  function (this: SiraWorld, name: string) {
    this.results =
      this.notifications.find(
        (n) => n.owner === name && n.type === "account_rejected",
      ) ?? null;
  },
);

Then(
  "{string} é avisado do motivo da recusa",
  function (this: SiraWorld, name: string) {
    assert.ok(
      this.notifications.some(
        (n) => n.owner === name && n.type === "account_rejected" && n.message,
      ),
    );
  },
);
...
Then(
  "ele lê o motivo informado pelo administrador",
  function (this: SiraWorld) {
...
When(
  "abro o painel pelo ícone de notificações no topo",
  function (this: SiraWorld) {
    const mine = this.notifications
      .filter((n) => n.owner === "Ana")
      .map(toNotifRow);
...
When("abro o painel de notificações", function (this: SiraWorld) {
  this.results = this.notifications
    .filter((n) => n.owner === "Ana")
    .map(toNotifRow);
});
...
Then(
  "vejo minhas notificações da mais recente para a mais antiga",
  function (this: SiraWorld) {
...
Then(
  "as lidas e não lidas aparecem visualmente diferenciadas",
  function (this: SiraWorld) {
    const rows = this.results as NotificationRow[];
...
Then(
  "o contador indica três notificações não lidas",
  function (this: SiraWorld) {
...
Given(
  "que tenho uma notificação por ler no painel",
  function (this: SiraWorld) {
    this.currentUser = ensureUser(this, "Ana");
    addNotification(this, {
      owner: "Ana",
...
Given(
  "que recebi uma notificação sobre minha reserva",
  function (this: SiraWorld) {
    this.currentUser = ensureUser(this, "Ana");
    const res = addReservation(this, {
      owner: "Ana",
      roomName: "Lab 1",
      status: "approved",
    });
...
Given(
  "que recebi um aviso geral sem reserva associada",
  function (this: SiraWorld) {
    this.currentUser = ensureUser(this, "Ana");
    addNotification(this, {
      owner: "Ana",
...
When("clico nessa notificação", function (this: SiraWorld) {
  ...
  n.is_read = true;
  this.focusReservation = n.related_reservation_id
    ? (this.reservations.find((r) => r.id === n.related_reservation_id) ?? null)
    : null;
...
Then(
  "o contador de não lidas diminui em uma unidade",
  function (this: SiraWorld) {
    const mine = this.notifications
      .filter((n) => n.owner === "Ana")
      .map(toNotifRow);
...
Then(
  "sou levado ao detalhe da reserva relacionada",
  function (this: SiraWorld) {
...
Then("ele é marcado como lido", function (this: SiraWorld) {
  assert.ok(
    this.notifications.filter((n) => n.owner === "Ana").every((n) => n.is_read),
  );
});
...
Given(
  "que {string} e {string} possuem notificações por ler",
  function (this: SiraWorld, a: string, b: string) {
    ensureUser(this, a);
    ensureUser(this, b);
...
When(
  "{string} marca todas as suas notificações como lidas",
  function (this: SiraWorld, name: string) {
    this.notifications
      .filter((n) => n.owner === name)
      .forEach((n) => (n.is_read = true));
...
Then("todas passam a constar como lidas", function (this: SiraWorld) {
  const name = this.currentUser?.name ?? "Ana";
  assert.ok(
    this.notifications.filter((n) => n.owner === name).every((n) => n.is_read),
  );
});
...
Then(
  "as notificações de {string} continuam por ler",
  function (this: SiraWorld, name: string) {
    const theirs = this.notifications
      .filter((n) => n.owner === name)
      .map(toNotifRow);
